...
const ShopByCategory = () => {
    ...
    const filterByPrice = (pri) => {
        setSelectedPrice(pri); // update selected price for UI
        setProducts(items.filter((pro) => pro.price <= pri));
      };
        ...
        <>
            <div className="container bg-dark text-light my-4 p-4 rounded">
                ...
                <div className="d-flex flex-wrap justify-content-center align-items-center gap-3 mb-4">
                    {categories.map(({ name, icons }) => <div className='d-flex align-items-center gap-3 px-3 py-2 button bg-secondary text-light fw-bold' style={{ cursor: 'pointer' }}  onClick={()=>filterByCategory(name)}>{icons} <span>{name}</span></div>)}
                </div>
                <div className="d-flex flex-wrap justify-content-center gap-3 ">
                    {priceRange.map((value) => <span className={`badge p-3 fw-bold ${selectedPrice === value ? "bg-warning text-dark" : "bg-light text-dark"} `} style={{ cursor: "pointer", fontSize: "1rem", borderRadius:"10px" }} onClick={()=>filterByPrice(value)}>
                        <FaRupeeSign /> {value}
                    </span>)}
                </div>
            </div>
